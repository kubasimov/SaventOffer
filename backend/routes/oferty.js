...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    // Audit log: zapisz zmienione pola
    if (stary) {
      const zmiany = [];
      ...
      if (req.body.klient_id !== undefined && String(stary.klient_id || '') !== String(req.body.klient_id || '')) {
        let nowaNazwa = req.body.klient_id, staraNazwa = stary.klient_id;
        try { const kr = await pool.query('SELECT nazwa FROM clients WHERE id=$1', [req.body.klient_id]); if (kr.rows.length) nowaNazwa = kr.rows[0].nazwa; } catch(e) {}
        try { const kr = await pool.query('SELECT nazwa FROM clients WHERE id=$1', [stary.klient_id]); if (kr.rows.length) staraNazwa = kr.rows[0].nazwa; } catch(e) {}
